...
订单创建后，业务应用需要按返回参数完成支付跳转或展示二维码，并轮询或刷新充值订单状态。`alipay_page` 和 `alipay_wap` 会返回 `pay_url`；`wechat_native` 会返回二维码内容；微信内的用户侧充值页会使用 JSAPI 支付。推荐直接跳转 Koffy 充值页，减少每个应用重复处理支付状态和异常恢复逻辑。
...
用户访问“充值”页选择人民币金额后，系统按 `1 元 = 100 点数` 创建支付订单。微信内使用 JSAPI 调起支付，微信外使用 Native 二维码弹窗；支付宝在电脑浏览器使用电脑网站支付跳转，在手机浏览器使用手机网站支付跳转。支付回调验签成功后自动入账，用户侧充值记录只展示已支付成功的记录，完整订单状态和订单号在管理后台查看。
